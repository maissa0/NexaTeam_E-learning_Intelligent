...
public class RecommandationServiceImpl implements RecommandationService {
    ...
    public Recommandation createRecommandation(Integer userId, Integer courseId, Integer engagement) {
    	   Optional<User> userOpt = userRepository.findById(userId);
    	    ...
    	    Optional<Recommandation> existingRecommandationOpt = recommandationRepository.findByUserIdAndCoursId(userId, courseId);
    	    
    	    Recommandation recommandation;
    	    if (existingRecommandationOpt.isPresent()) {
    	        recommandation = existingRecommandationOpt.get();
    	        recommandation.setEngagement(recommandation.getEngagement() + engagement);
	...
	@Override
	public List<Cours> recommendCoursesByEngagement(Integer id) {
	    // Prepare the request body
	    Map<String, Integer> requestBody = Map.of("user_id", id);
	    
	    HttpHeaders headers = new HttpHeaders();
	    headers.setContentType(MediaType.APPLICATION_JSON);
	    HttpEntity<Map<String, Integer>> entity = new HttpEntity<>(requestBody, headers);

	    // Call the Flask API to get recommended courses
	    String flaskApiUrl = "http://localhost:5001/recommend";
	    RestTemplate restTemplate = new RestTemplate();

	    // Exchange POST request and get response
	    ResponseEntity<List> response = restTemplate.exchange(
	        flaskApiUrl, HttpMethod.POST, entity, List.class
	    );

	    // Extract the list of recommended courses from the response
	    List<Map<String, Object>> recommendedCourses = response.getBody();
	    
	    // Convert the response back to Cours objects
	    List<Cours> courses = new ArrayList<>();
	    for (Map<String, Object> courseData : recommendedCourses) {
	        Integer courseId = (Integer) courseData.get("cours_id");
