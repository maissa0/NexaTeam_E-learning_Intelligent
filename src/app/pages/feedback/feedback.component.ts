...
export class FeedbackComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private feedbackService: FeedbackService, private recommondationService: RecommondationService) {}
  toggleShowMore() {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params => {
      ...
      if (id) {
        this.courseId = +id; 
        this.loadFeedbacks();
        this.recommondationService.addEngagement(this.userId,+id,1).subscribe();
      }
    });
  }
  ...
  submitFeedback(): void {
    ...
    this.feedbackService.createFeedback(feedbackData).subscribe({
      next: (response) => {
        ...
        this.recommondationService.addEngagement(this.userId,this.courseId,5).subscribe();
  ...
  toggleLike(feedbackId: number): void {
    this.feedbackService.toggleLike(feedbackId, this.userId).subscribe({
      next: (updatedFeedback) => {
        ...
        if (updatedFeedback.likedByUsers.some((user: { id: number; }) => user.id === this.userId)) {
          this.recommondationService.addEngagement(this.userId,this.courseId,3).subscribe();
        }
