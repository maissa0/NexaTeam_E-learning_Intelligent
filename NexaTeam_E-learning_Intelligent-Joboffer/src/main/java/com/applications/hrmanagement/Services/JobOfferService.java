...
public class JobOfferService implements IjobOfferService {
    ...
    public jobOffer createJobOffer(JobOfferDto jobOfferDTO) {
        // Si la description n'est pas fournie, on la génère automatiquement
        if (jobOfferDTO.getDescription() == null || jobOfferDTO.getDescription().trim().isEmpty()) {
            String generatedDescription = aiDescriptionService.generateDescription(jobOfferDTO);
            jobOfferDTO.setDescription(generatedDescription);
        }
